...
``experiments/tests/`` has no ``__init__.py`` (deliberately -- it isn't part
...
``from experiments.tests.helpers import synthetic_molecule_set`` (an
implicit-namespace-package import: ``experiments/`` has no ``__init__.py``
...
breaks running ``pytest experiments/tests/`` on its own.
"""
