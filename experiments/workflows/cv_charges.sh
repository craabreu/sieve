...
# Figures are tracked on purpose (FIGURES_DIR below), while experiments/results
# is gitignored, so a step that regenerates one leaves the working tree dirty.
# Every fitting and CV entry point then calls cv._check_clean, which refuses to
# run against a dirty tree -- correctly, since a run's manifest records the
# commit it ran under and a dirty tree makes that record a lie. The two
# policies collide in the middle of a campaign: depth-curve dirtied the tree
# and study-b-dash died on it, and compare would have done the same to
# study-c-shard-fits.
#
# Committing the regenerated figures as soon as the step that produced them
# finishes resolves it in the direction that keeps provenance honest, rather
# than by passing allow_dirty and marking every downstream run dirty. Only
# FIGURES_DIR is staged, so nothing else in the tree is swept in; a step that
# touches anything else still stops the workflow, which is what should happen.
commit_figures() {
  local step_name=$1
  # porcelain, not `git diff --quiet`: a step's first output is an untracked
  # file, which diff does not see (Study D's report is one).
  [ -z "$(git status --porcelain -- "$FIGURES_DIR")" ] && return 0
  git add -- "$FIGURES_DIR" || return 0
  git commit -q -m "results(experiments): figures regenerated by $step_name

Committed by the workflow so the next step's cv._check_clean sees a clean
tree. experiments/docs/figures is tracked while experiments/results is not,
so a figure-producing step would otherwise stop the campaign." || true
  echo "== commit $step_name figures"
}
...
# ===========================================================================
# Study D: the stereo-aware Sieve
# ===========================================================================
#
# docs/superpowers/specs/2026-09-23-stereo-refines-the-blind-class-design.md
# section 11. Element continuation with the cis/trans track, whose trit
# refines the stereo-blind class of each WL level, against the same model
# without it, at the incumbent's selected depth, five repeats of K folds.
#
# The question is local: does the track improve the atoms stereo can reach?
# The primary metric is therefore RMSE within two bonds of a stereogenic
# double bond (near_ez2/rmse); corpus-wide RMSE and MAE are the
# no-regression check.
#
# Two estimators, both read from one set of shard fits: continuation without
# shrinkage, the estimator the manuscript calls the method, and continuation
# with empirical-Bayes shrinkage, the only setting under which the aware-only
# classes' shrinkage toward their blind counterpart acts at all. Neither
# incumbent is re-run: Study B already scored both at this depth over the
# same repeats, store, shard partition and K, with predictions saved, so its
# runs are this study's paired arm, as they were for Study C.
#
# Fitted at the selected depth, not SIEVE_MAX_DEPTH: nothing here reads a
# deeper level, and a stereo-aware fit holds up to two classes per atom.
STUDY_D_CONFIG_LABEL=element-ct-eb  # the FIT applies empirical_bayes, as SIEVE_CONFIG_LABEL's
STUDY_D_DEPTH="$SIEVE_SELECTED_DEPTH"
STUDY_D_REPEATS="$STUDY_B_REPEATS"
SIEVE_STUDY_D=sieve-cv-study-d
STUDY_D_PARAMS=$(
  echo "$SIEVE_PREDICTOR_PARAMS" | "$PYTHON" -c '
import json, sys
params = json.load(sys.stdin)
params["stereo"] = ["cis_trans"]
print(json.dumps(params))
'
)
STUDY_D_VARIANTS='[
  {"method": "sieve-element-ct-continuation",    "class_estimator": "continuation", "shrinkage_weight": null},
  {"method": "sieve-element-ct-continuation-eb", "class_estimator": "continuation", "shrinkage_weight": "empirical_bayes"}
]'
STUDY_D_METHODS=sieve-element-ct-continuation,sieve-element-ct-continuation-eb
...
fit_one_study_d_shard() {
  "$PYTHON" -m experiments cv-fit-sieve-shards "$STORE" \
    ...
    --codes-path "$CODES_PATH" --config-label "$STUDY_D_CONFIG_LABEL" \
    --predictor-params "$STUDY_D_PARAMS" \
    $COLLAPSE_FLAG
}
export -f fit_one_study_d_shard
export STUDY_D_DEPTH CODES_PATH STUDY_D_CONFIG_LABEL STUDY_D_PARAMS

dispatch_study_d_shards() {
  all_shard_ids | xargs -P "$STUDY_D_SHARD_JOBS" -n 1 \
    bash -c 'fit_one_study_d_shard "$1"' --
}

step study-d-shard-fits \
  "shard_fits_count_is fit-sieve-$STUDY_D_CONFIG_LABEL-w$STUDY_D_DEPTH-s $N_SHARDS" -- \
  dispatch_study_d_shards
...
run_study_d_repeat() {
  "$PYTHON" -m experiments cv-run-sieve "$STORE" \
    ...
    --predictor-params "$STUDY_D_PARAMS" \
    --variants "$STUDY_D_VARIANTS" \
    $MODEL_CACHE_FLAG \
    --normalization equal_weighted --method sieve-element-ct-continuation \
    $COLLAPSE_FLAG \
    --experiment "$SIEVE_STUDY_D" \
    --save-predictions
}
export -f run_study_d_repeat
export SIEVE_STUDY_D STUDY_D_VARIANTS

study_d_runs_done() {
  local m
  for m in $(echo "$STUDY_D_METHODS" | tr ',' ' '); do
    method_depth_runs_count_is "$SIEVE_STUDY_D" "$m" "$STUDY_D_DEPTH" \
      "$((K * $(n_items "$STUDY_D_REPEATS")))" || return 1
  done
}

dispatch_study_d_repeats() {
  echo "$STUDY_D_REPEATS" | tr ',' '\n' \
    | xargs -P "$STUDY_D_JOBS" -n 1 bash -c 'run_study_d_repeat "$1"' --
}

step study-d-runs "study_d_runs_done" -- dispatch_study_d_repeats
...
step study-d-subset-scores \
  "'$PYTHON' -m experiments score-stereo-subsets '$STORE' $STUDY_D_SELECT --depth $STUDY_D_DEPTH --check" -- \
  "$PYTHON" -m experiments score-stereo-subsets "$STORE" $STUDY_D_SELECT \
    --depth "$STUDY_D_DEPTH"

STUDY_D_REPORT="$FIGURES_DIR/study-d"
STUDY_D_METRICS="near_ez2/rmse near_ez1/rmse has_ez/rmse near_ez2/mae rmse mae"
...
run_study_d_report() {
  ...
  for m in $STUDY_D_METRICS; do metric_flags="$metric_flags --metric $m"; done
