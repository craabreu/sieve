...
``charges_experiments/tests/`` has no ``__init__.py`` (deliberately -- it isn't part
...
``from charges_experiments.tests.helpers import synthetic_molecule_set`` (an
implicit-namespace-package import: ``charges_experiments/`` has no ``__init__.py``
...
breaks running ``pytest charges_experiments/tests/`` on its own.
"""
