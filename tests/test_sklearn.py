...
def test_get_set_params_round_trip():
    r = SieveRegressor(simple_config(), minimum_support=3, shrinkage_strength=1.0)
    assert r.get_params()["minimum_support"] == 3
    r.set_params(minimum_support=9)
    assert r.get_params()["minimum_support"] == 9
...
def test_graph_kfold_covers_every_atom_exactly_once_as_test():
    ...
    assert np.array_equal(np.sort(seen), np.arange(b.n_nodes))
...
def test_cross_val_score_actually_works():
    ...
    r.fit(batch, batch.y) directly, so this exercises NodeBatch's own
...
def test_grid_search_cv_sweeps_minimum_support_and_shrinkage_strength():
    ...
    search = GridSearchCV(
        SieveRegressor(cfg),
        param_grid={"minimum_support": [1, 2], "shrinkage_strength": [None, 1.0]},
        cv=GraphKFold(n_splits=3, random_state=2),
    )
    search.fit(b, b.y)
    assert search.best_params_["minimum_support"] in (1, 2)
